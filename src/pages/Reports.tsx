...
const reports: { key: ReportKey; title: string; detail: string; classificationLabel: string; icon: typeof ClipboardList }[] = [
  { key: 'service-orders', title: 'Ordens de serviço', detail: 'Cliente, agenda, status, categoria e valor cobrado', classificationLabel: 'Status', icon: ClipboardList },
  { key: 'clients', title: 'Clientes e contratos', detail: 'Documento, tipo de pessoa, quantidade de OS e valor acumulado', classificationLabel: 'Tipo', icon: UsersRound },
  { key: 'invoices', title: 'Notas fiscais', detail: 'Competência, emissão, situação e valor total', classificationLabel: 'Status', icon: ReceiptText },
  { key: 'statements', title: 'Movimentos de extrato', detail: 'Conta, data, créditos, débitos e saldo', classificationLabel: 'Tipo', icon: CircleDollarSign },
]
...
export function Reports() {
  ...
  const reportRows = useMemo<ReportRow[]>(() => {
    let rows: ReportRow[] = []
    if (selected === 'clients') rows = (clientsQuery.data?.content ?? []).map((client) => ({ code: `CLI-${client.id}`, description: `${client.name || 'Sem nome'} · ${client.serviceOrderCount} OS`, date: '', status: enumLabel(client.kind), value: Number(client.totalValue ?? 0) }))
  ...
  function exportCsv() {
    ...
    const header = ['Código', 'Descrição', 'Data', selectedReport.classificationLabel, 'Valor']
